...
* Notice: the errors and the RF models generated each time would be slightly different from the ones here. This is the outcome I got when I was runnnig this demonstration. Alternatively, you can use the function "set.seed()" to fix the outcome each time you produce so that the result would be reproducible.
...
From the plot, we know that as more and more trees are built, the errors decrease and eventually become stable. It seems that when the number of trees is around 10-50, we arrived at the lowest error. We can check it with the “which.min” function.
...
It turns out that we can have the lowest error in the RF model with 21 trees, so let’s rebuild the model with this number of trees by using the argument “ntree”.
...
The models with the number of variables is one does the best since it has the minimum error. The variance explained also increase a bit. Hence, we will use the one with mtry = 1. 
...
Overall, this RF model suggests that the all abiotic variables (SST, light at the surveyed depth, and chlorophyll *a*) positively contribute to the cover of *S. pistillata*. Among these variables, the light is the strongest predictor. With these three factors, we can already catch 32.1% of the total variance. Nonetheless, considering the number of observations and also the number of variables, this dataset is too small to build a very robust RF model. Further sample effort is needed to make this prediction stronger especially in places other than the shallow water in the north of Taiwan. 
